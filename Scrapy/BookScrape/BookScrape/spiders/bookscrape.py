...
from BookScrape.items import BookscrapeItem
...
class BookscrapeSpider(scrapy.Spider):
    ...
    def parse_book(self, response):
        book = BookscrapeItem()  # 将信息存入BooksItem对象
        ...
        book['stock'] = response.xpath('//table[@class="table table-striped"]//tr[6]/td/text()').re_first(
            '\((\d+) available\)')
